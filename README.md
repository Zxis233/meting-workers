...
`SECRET` 是关键配置，按你的需求，它保存允许访问的域名列表，逗号分隔：
...
| 变量名 | 说明 | 默认值 |
| --- | --- | --- |
| `SECRET` | 允许访问的域名白名单，逗号分隔 | 无 |
| `DEBUG_ALLOW_ALL_REFERERS` | 本地调试时跳过 `Origin/Referer` 校验 | `false` |
| `AUTH_SECRET` | 可选，开启后会给 `url/pic/lrc` 自动加签并校验 `auth` | 空 |
...
如果 Cookie 不敏感或者你只做测试，也可以直接写进 `[vars]`，但不推荐。
...
- 如果配置了 `AUTH_SECRET`，会仿照原项目对 `url` / `pic` / `lrc` 生成 HMAC-SHA1 `auth` 参数。
