...
| `NETEASE_COOKIE_REFRESH_ENABLED` | 可选，是否启用 Cron 自动刷新网易云 Cookie                       | `true`                |
...
Worker 运行时不能写回 `NETEASE_COOKIE` Secret，因此自动刷新使用 KV 持久化：
...
4. 刷新成功后，把上游返回的 `Set-Cookie` 合并到当前 Cookie，再只提取 `MUSIC_U` 写回 KV。
...
如果同时配置了 `TELEGRAM_BOT_ID` 和 `TELEGRAM_CHAT_ID`，Cookie 刷新成功或失败时会向对应 Chat 推送关键信息，包括时间、状态码、Cron 信息、KV 绑定名和网易云上游状态；失败时会额外包含错误原因。推送内容不会包含完整 Cookie 或 Bot Token。
...
- `NETEASE_COOKIE` 是初始登录 Cookie；刷新后的 Cookie 只提取 `MUSIC_U` 并保存在 KV 中
- `TELEGRAM_BOT_ID`、`TELEGRAM_CHAT_ID` 是可选配置；两者都配置后才会启用刷新成功/失败推送
