...
如果同时配置了 `TELEGRAM_BOT_ID` 和 `TELEGRAM_CHAT_ID`，Cookie 定时检查和刷新会向对应 Chat 推送结果。成功时只推送简略信息，例如时间、核心 HTTP 状态和触发方式；失败时保留详细排查信息，例如检查接口、Location、真实音频响应或网易云上游状态。推送内容不会包含完整 Cookie、Bot Token 或未脱敏的 `auth` 参数。
